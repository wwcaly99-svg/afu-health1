...
export default function PlanDetailDrawer({
  ...
  const total = plan.tasks ? plan.tasks.length : plan.items?.length || 0;
  const done = plan.tasks
    ? plan.tasks.filter((_, i) => checkedItems.has(i)).length
    : plan.items?.filter((_, i) => checkedItems.has(i)).length || 0;
  const progress = total > 0 ? Math.round((done / total) * 100) : 0;

  const taskList = plan.tasks || plan.items?.map((text, i) => ({ id: `t${i}`, text })) || [];
      ...
      <div className="drawer" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="drawer-content">
          ...
          {plan.subtitle && !plan.description && (
            <p className="drawer-desc">{plan.subtitle}</p>
          )}
          {plan.duration && (
